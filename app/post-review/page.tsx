...
const contextTags: Record<string, { icon: string; label: string }[]> = {
  Restaurant: [
    { icon: "🍽️", label: "Just dining" },
    { icon: "✨", label: "Date night" },
    { icon: "🥂", label: "Celebration" },
    { icon: "🎁", label: "Someone's guest" },
  ],
  "Fine Dining": [
    { icon: "🍽️", label: "Just dining" },
    { icon: "✨", label: "Date night" },
  ...
  Fitness: [
    ...
    { icon: "🔄", label: "Regular member" },
    { icon: "🎁", label: "Guest of a member" },
    { icon: "🏆", label: "Working toward a goal" },
  ],
  Retail: [
    { icon: "🛒", label: "Weekly shop" },
    { icon: "🎁", label: "Buying for someone" },
    { icon: "🌟", label: "Treating myself" },
    { icon: "🔍", label: "Just browsing" },
  ],
  Grocery: [
    { icon: "🛒", label: "Weekly shop" },
    { icon: "🎁", label: "Buying for someone" },
  ...
  Services: [
    { icon: "✂️", label: "Regular visit" },
    { icon: "🆕", label: "First time here" },
    { icon: "🎁", label: "Gift for someone" },
    { icon: "💼", label: "Special occasion" },
  ],
  Health: [
    { icon: "💪", label: "First class" },
    { icon: "🔄", label: "Regular member" },
...
function ReviewForm() {
  ...
  const tags = selectedBiz
    ? contextTags[selectedBiz.category] || contextTags.default
    : contextTags.default
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from("reviews").insert({
      ...
      status: stars < 4 ? "pending" : "published",
    })
  ...
  if (submitted) return (
    <div style={{ fontFamily: "sans-serif", maxWidth: "430px", margin: "0 auto", minHeight: "100vh", background: "#f7f7f5", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem", textAlign: "center" }}>
      ...
      <div style={{ fontSize: "22px", fontWeight: "700", marginBottom: "8px" }}>Review submitted!</div>
      <div style={{ fontSize: "14px", color: "#888", marginBottom: "16px", lineHeight: "1.6" }}>
        {stars < 4
          ? "Your review is being verified. The business has 72 hours to respond before it goes public."
          : "Your review is now live on Reviu. Thank you for helping others!"}
      </div>
      <div style={{ background: "#EEEDFE", borderRadius: "12px", padding: "12px 16px", marginBottom: "24px", fontSize: "13px", color: "#3C3489", lineHeight: "1.6", width: "100%" }}>
        Your legitimacy score has been calculated and attached to this review.
      </div>
          ...
          {!selectedBiz ? (
            <>
              <input
                ...
                style={{ width: "100%", padding: "12px 16px", borderRadius: "12px", border: "1px solid #e5e5e5", fontSize: "14px", background: "#f7f7f5", boxSizing: "border-box", marginBottom: "10px" }}
              />
              {businesses
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {!bizId && (
                <div onClick={() => { setSelectedBiz(null); setContextTag("") }} style={{ fontSize: "12px", color: "#534AB7", cursor: "pointer", fontWeight: "600" }}>Change</div>
              )}
            </div>
          )}
        </div>
          ...
          <>
            <div style={{ background: "white", borderRadius: "16px", padding: "1.25rem", marginBottom: "12px", border: "1px solid #eee" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                {tags.map(tag => (
                  <div
                    key={tag.label}
                    onClick={() => setContextTag(tag.label)}
                    style={{ padding: "12px", borderRadius: "12px", border: contextTag === tag.label ? "2px solid #534AB7" : "1px solid #eee", background: contextTag === tag.label ? "#EEEDFE" : "#f7f7f5", cursor: "pointer", textAlign: "center" }}
                  >
              ...
              <div style={{ display: "flex", gap: "10px", justifyContent: "center", marginBottom: "8px" }}>
                {[1, 2, 3, 4, 5].map(star => (
                  <span
                    ...
                    style={{ fontSize: "40px", cursor: "pointer", color: star <= (hover || stars) ? "#f59e0b" : "#ddd", transition: "color 0.1s" }}
                  >★</span>
                ))}
              </div>
              <div style={{ textAlign: "center", fontSize: "13px", color: "#888", fontWeight: "500" }}>
                {stars === 0 && "Tap to rate"}
                {stars === 1 && "Poor"}
                {stars === 2 && "Fair"}
                {stars === 3 && "Good"}
            ...
            <div style={{ background: "white", borderRadius: "16px", padding: "1.25rem", marginBottom: "12px", border: "1px solid #eee" }}>
              ...
              <textarea
                ...
                placeholder="Share your honest perspective. Every experience is valid — yours matters here."
                style={{ width: "100%", padding: "12px 16px", borderRadius: "12px", border: "1px solid #e5e5e5", fontSize: "14px", background: "#f7f7f5", minHeight: "130px", resize: "none", boxSizing: "border-box", lineHeight: "1.6", fontFamily: "sans-serif" }}
              />
            ...
            <div style={{ background: "white", borderRadius: "16px", padding: "1.25rem", marginBottom: "12px", border: "1px solid #eee" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div
                  ...
                  style={{ width: "48px", height: "28px", borderRadius: "14px", background: isFirstVisit ? "#534AB7" : "#ddd", cursor: "pointer", position: "relative", transition: "background 0.2s", flexShrink: 0 }}
                >
            ...
            {error && (
              <div style={{ background: "#FCEBEB", color: "#A32D2D", padding: "12px 16px", borderRadius: "10px", fontSize: "13px", marginBottom: "12px" }}>
                {error}
              </div>
            )}
            ...
            <div
              ...
              style={{ background: submitting ? "#9990D9" : "#534AB7", color: "white", padding: "16px", borderRadius: "14px", fontSize: "15px", fontWeight: "600", textAlign: "center", cursor: submitting ? "not-allowed" : "pointer" }}
            >
